...
export default function EntertainerList() {
    ...
    const adminAPI = (url = 'https://prideweddingapi.azurewebsites.net/api/EntertainmentVendors/') => {
        return {
        ...
        <div className="row">
            <div className="col-md-12">
                <div className="jumbotron jumbotron-fluid py-4">
                    ...
                    <br></br>
                        <h1 className="display-4">Entertainment Vendors</h1>
                    </div>
                </div>
            </div>
            ...
            <h1 className="lead">Available Advertisements</h1>
                <table>
                    <tbody>
                        {
                            //tr > 3 td
                            [...Array(Math.ceil(entertainerList.length / 4))].map((e, i) =>
                                <tr key={i}>
                                    <td>{imageCard(entertainerList[4 * i])}</td>
                                    <td>{entertainerList[4 * i + 1] ? imageCard(entertainerList[4 * i + 1]) : null}</td>
                                    <td>{entertainerList[4 * i + 2] ? imageCard(entertainerList[4 * i + 2]) : null}</td>
                                    <td>{entertainerList[4 * i + 3] ? imageCard(entertainerList[4 * i + 3]) : null}</td>
                                </tr>
                            )
                        }
                    </tbody>
